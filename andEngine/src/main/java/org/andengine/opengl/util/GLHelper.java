...
public class GLHelper {
	...
	public static Buffer getPixels(final Bitmap pBitmap, final PixelFormat pPixelFormat, final ByteOrder pByteOrder) {
		final int[] pixelsARGB_8888 = GLHelper.getPixelsARGB_8888(pBitmap);		//得到一个写入了Bitmap像素数据的数组
	...
	private static int[] pixelsARGB_8888;

	public static int[] getPixelsARGB_8888(final Bitmap pBitmap) {
		final int w = pBitmap.getWidth();
		final int h = pBitmap.getHeight();

		if(pixelsARGB_8888 == null){
			pixelsARGB_8888 = new int[w * h];	//改动标记，pixelsARGB_8888数组改成了静态
		}

		pBitmap.getPixels(pixelsARGB_8888, 0, w, 0, 0, w, h);	//这个方法书要是为了将Bitmap的图像像素数据写入pixelsARGB_8888[]这个数组中
